...
cloudevents_absl::StatusOr<std::unique_ptr<StructuredCloudEvent>> JsonFormatter::Serialize(
  ...
  for (auto const& attr : *attrs) {
    cloudevents_absl::StatusOr<Json::Value> json_printed = PrintToJson(attr.second);
    if (!json_printed.ok()) {
